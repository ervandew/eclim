...
" Script Variables {{{
  let s:validate_command = '-command css_validate -p "<project>" -f "<file>"'
" }}}

" Validate(on_save) {{{
" Validates the current file.
function! eclim#css#validate#Validate (on_save)
  if a:on_save && (!g:EclimCssValidate || eclim#util#WillWrittenBufferClose())
    return
  endif
