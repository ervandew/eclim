...
"
" Description: {{{
"   see http://eclim.sourceforge.net/vim/java/webxml/validate.html
"
" License:
"
...
" Script Variables {{{
  let s:validate_command = '-command webxml_validate -p "<project>" -f "<file>"'
" }}}

" Validate(on_save) {{{
" Validates the current file.
function! eclim#java#webxml#validate#Validate (on_save)
  if a:on_save && (!g:EclimWebXmlValidate || eclim#util#WillWrittenBufferClose())
    return
  endif
