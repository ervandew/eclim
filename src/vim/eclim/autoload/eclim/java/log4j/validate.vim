...
" Script Variables {{{
  let s:validate_command = '-command log4j_validate -p "<project>" -f "<file>"'
" }}}

" Validate(on_save) {{{
" Validates the current file.
function! eclim#java#log4j#validate#Validate (on_save)
  if a:on_save && (!g:EclimLog4jValidate || eclim#util#WillWrittenBufferClose())
    return
  endif
