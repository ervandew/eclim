...
" Script Variables {{{
  let s:update_command = '-command c_src_update -p "<project>" -f "<file>"'
" }}}

" UpdateSrcFile(validate) {{{
" Updates the src file on the server w/ the changes made to the current file.
function! eclim#c#util#UpdateSrcFile(validate)
  ...
  if project != ""
    ...
    if (g:EclimCValidate || a:validate) && !eclim#util#WillWrittenBufferClose()
