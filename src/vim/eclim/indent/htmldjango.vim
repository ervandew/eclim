...
" [-- count indent-decreasing tags of line a:lnum --]
fun! <SID>DjangoHtmlIndentClose(lnum)
  let s = substitute('x'.getline(a:lnum),
        \ '.\{-}\({%\s*end[a-z]\+\s*%}\)', "\1", 'g')
  let s = substitute(s, "[^\1].*$", '', '')
  return strlen(s)
...
" invoked in HtmlIndentSum to handle additional indenting of attributes on an
" element much like indenting parameters in a java function call.
fun! <SID>HtmlIndentAttributeWrap (lnum)
  let lnum = a:lnum
  let start = 0
  let end = 0
...
fun! <SID>HtmlIndentSum(lnum, style)
  ...
  if a:style == match(getline(a:lnum), '^\s*{%\s*end')
    let open = <SID>DjangoHtmlIndentOpen(a:lnum)
    let close = <SID>DjangoHtmlIndentClose(a:lnum)
    if 0 != open || 0 != close
      return open - close
    endif
  endif
  " couple exceptions
  if a:style == -1 && getline(a:lnum) =~ '^\s*{%\s*\(else\|elif\)'
    return 1
  endif
  if a:style == 0 && getline(a:lnum) =~ '^\s*{%\s*\(else\|elif\)'
    return -1
  endif
  " #### END EV ADDED ####
  if '' != &syntax &&
...
fun! HtmlIndentGet(lnum)
  ...
  if 0 < searchpair(js, '', '</script>', 'nWb')
    ...
    if getline(lnum) !~ js && getline(a:lnum) !~ '</script>'
      ...
      return indent != 0 ? indent : indent(lnum)
    endif
  endif
