...
function! TestRenameField()
  ...
  call VUAssertEquals(line(1), '|diff|: ' . getcwd() . '/src/org/eclim/test/refactoring/rename/vn1/TestN1VUnit.java', 'wrong preview line 1')
  call VUAssertEquals(line(2), '|diff|: ' . getcwd() . '/src/org/eclim/test/refactoring/rename/vn1/vn2/TestN2VUnit.java', 'wrong preview line 2')
  ...
  call VUAssertEquals(expand('%'), 'TestN1VUnit.current.java', 'wrong diff current 1')
  call VUAssertTrue(&diff, 'current diff not enabled 1')
  call VUAssertEquals(getline(5),
    \ '  public static final String FOO = "value";', 'current FOO field incorrect 1')
  winc l
  call VUAssertEquals(expand('%'), 'TestN1VUnit.new.java', 'wrong diff new 1')
  call VUAssertTrue(&diff, 'new diff not enabled 1')
  call VUAssertEquals(getline(5),
    \ '  public static final String BAR = "value";', 'new FOO field incorrect 1')

  let numwins = winnr('$')
  winc j
  call VUAssertEquals(expand('%'), '[Refactor Preview]', 'wrong preview title 2')
  call cursor(2, 1)
  exec "normal \<cr>"
  call VUAssertEquals(winnr('$'), numwins, 'wrong number of windows')

  call VUAssertEquals(expand('%'), 'TestN2VUnit.current.java', 'wrong diff current 2')
  call VUAssertTrue(&diff, 'current diff not enabled 2')
  call VUAssertEquals(getline(5),
    \ 'import static org.eclim.test.refactoring.rename.vn1.TestN1VUnit.FOO;',
    \ 'current import FOO field incorrect.')
  call VUAssertEquals(getline(11),
    \ '    System.out.println(FOO);', 'current FOO reference incorrect 1')
  winc l
  call VUAssertEquals(expand('%'), 'TestN2VUnit.new.java', 'wrong diff new 2')
  call VUAssertTrue(&diff, 'new diff not enabled 2')
  call VUAssertEquals(getline(5),
  ...
  call VUAssertEquals(lines[4],
    \ 'import static org.eclim.test.refactoring.rename.vn1.TestN1VUnit.BAR;',
    \ 'static field import rename incorrect')
  call VUAssertEquals(lines[10],
...
function! TestRenameType()
  ...
  call VUAssertEquals(line(2), " other: Rename compilation unit 'TestN1VUnit.java' to 'TestR1VUnit.java'", 'wrong preview line 2')
  call VUAssertEquals(line(3), '', 'wrong preview line 3')
  call VUAssertEquals(line(4), '|Execute Refactoring|', 'wrong preview line 4')
  ...
  call VUAssertEquals(expand('%'), 'TestN2VUnit.current.java', 'wrong diff current 1')
  call VUAssertTrue(&diff, 'current diff not enabled 1')
  ...
  call VUAssertEquals(getline(12),
    \ '    TestN1VUnit test = new TestN1VUnit();',
    \ 'current type reference incorrect 1')
  winc l
  call VUAssertEquals(expand('%'), 'TestN2VUnit.new.java', 'wrong diff new 1')
  call VUAssertTrue(&diff, 'new diff not enabled')
  call VUAssertEquals(getline(3),
